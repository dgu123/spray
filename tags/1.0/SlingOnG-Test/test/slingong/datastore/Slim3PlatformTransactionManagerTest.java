...
import slingong.datastore.Slim3PlatformTransactionManager.GlobalTransactionState;
import slingong.datastore.testmodel.book.Author;
import slingong.datastore.testmodel.book.AuthorBook;
import slingong.datastore.testmodel.book.AuthorChapter;
...
*/
	...
	public void setUp() throws Throwable {
		...
		Assert.assertTrue( applicationContext.containsBean( "annotationBeanConfigurerAspect"));
		Assert.assertNotNull( applicationContext.getBean( "annotationBeanConfigurerAspect"));
		
		Assert.assertTrue( applicationContext.containsBean( "txManager"));
		Assert.assertEquals( 
				applicationContext.getBean( "txManager"), slim3PlatformTransactionManager
				);
		Assert.assertTrue( slim3PlatformTransactionManager instanceof Slim3PlatformTransactionManager);
/* Changed default transaction behavior setting
		Assert.assertEquals( 
...
/*
		Assert.assertTrue( applicationContext.containsBean( "annotationTransactionAspect"));
		Assert.assertNotNull( applicationContext.getBean( "annotationTransactionAspect"));
*/
		Assert.assertTrue( applicationContext.containsBean( "slim3AnnotationTransactionAspect"));
		Assert.assertNotNull( applicationContext.getBean( "slim3AnnotationTransactionAspect"));
		// ----------------------------------------------------------------------------------------
...
*/
	...
	protected void throwConcurrentModification() throws Throwable {
		...
		slingong.datastore.testmodel.book.ChapterMeta chapterMeta
		= slingong.datastore.testmodel.book.ChapterMeta.get();
	...
	public void testNoRollbackByException() throws Throwable {
		...
		slingong.datastore.testmodel.book.BookMeta bookMeta 
		= slingong.datastore.testmodel.book.BookMeta.get();
		List<Book> bookList 
